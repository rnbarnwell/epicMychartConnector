...
> Restored documentation file (including Mermaid diagram) after accidental revert/removal.
...
## 5) Sequence diagram: login + get allergies

The following Mermaid sequence diagram is GitHub-compatible (`sequenceDiagram`) and shows a complete happy-path flow from login through allergy retrieval.
...
sequenceDiagram
    ...
    participant R as MyChartRequest (cookie jar)
    participant M as MyChart Server

    U->>S: myChartUserPassLogin(hostname, user, pass)
    S->>R: determineFirstPathPart()
    R->>M: GET https://{hostname}
    M-->>R: 302 Location /{firstPathPart}/...
    R-->>S: firstPathPart resolved

    S->>R: GET /Authentication/Login
    R->>M: GET /{firstPathPart}/Authentication/Login
    M-->>R: Login HTML + __RequestVerificationToken + cookies
    R-->>S: login page HTML

    S->>R: POST /Authentication/Login/DoLogin (form-urlencoded)
    R->>M: DoLogin with LoginInfo + CSRF
    M-->>R: SecondaryValidation page OR Home page
    R-->>S: login response

    alt 2FA required
        S->>R: POST /Authentication/SecondaryValidation/SendCode
        R->>M: SendCode (email/SMS)
        M-->>R: Code sent acknowledgement
        R-->>S: need_2fa

        U->>S: complete2faFlow(code)
        S->>R: GET /Authentication/SecondaryValidation
        R->>M: SecondaryValidation page (fresh CSRF)
        M-->>R: HTML with token
        S->>R: POST /Authentication/SecondaryValidation/Validate
        R->>M: Validate TwoFactorCode
        M-->>R: { Success: true }
    end

    opt Terms & Conditions gate
        S->>R: GET /Authentication/TermsConditions
        R->>M: Load T&C form
        M-->>R: HTML form + hidden fields
        S->>R: POST TermsConditions acceptance
        R->>M: Submit hidden fields
        M-->>R: Redirect/Home
    end

    U->>S: getAllergies(mychartRequest)
    S->>R: GET /Clinical/Allergies
    R->>M: Bootstrap allergies page (token/cookies context)
    M-->>R: Allergies HTML

    S->>R: POST /api/allergies/LoadAllergies
    R->>M: JSON API call with CSRF/session cookies
    M-->>R: Allergies JSON payload
    R-->>S: AllergiesResult { allergies, allergiesStatus }
    S-->>U: AllergiesResult
```
